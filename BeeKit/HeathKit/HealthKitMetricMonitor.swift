...
class HealthKitMetricMonitor {
  ...
  public func registerObserverQuery() {
    ...
    let query = HKObserverQuery(
      sampleType: metric.sampleType(), predicate: nil,
      updateHandler: { (query, completionHandler, error) in
        ...
        guard error == nil else {
          self.logger.error(
            "ObserverQuery for \(self.metric.databaseString, privacy: .public) was error: \(error, privacy: .public)")
          return
        }
        ...
        if let lastUpdate = self.lastObserverUpdate {
          if Date().timeIntervalSince(lastUpdate) < HealthKitMetricMonitor.minimumIntervalBetweenObserverUpdates {
            self.logger.notice(
              "Ignoring update to \(self.metric.databaseString, privacy: .public) due to recent previous update")
            completionHandler()
            return
          }
        }
  ...
  public func unregisterObserverQuery() {
    guard let query = self.observerQuery else {
      logger.warning(
        "unregisterObserverQuery(\(self.metric.databaseString, privacy: .public)): Attempted to unregister query when not registered"
      )
      return
    }
