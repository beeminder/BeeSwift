...
class QuantityHealthKitMetric: HealthKitMetric {
  ...
  internal init(
    humanText: String, databaseString: String, category: HealthKitCategory,
    hkQuantityTypeIdentifier: HKQuantityTypeIdentifier, precision: [HKUnit: Int] = [:]
  ) {
    ...
    async throws -> [BeeDataPoint]
  {
    ...
    let statsCollection = try await withCheckedThrowingContinuation {
    ...
    return try await datapointsForCollection(
      collection: statsCollection, startDate: startDate, endDate: today, deadline: deadline, healthStore: healthStore)
  }
  ...
  private func datapointsForCollection(
    collection: HKStatisticsCollection, startDate: Daystamp, endDate: Daystamp, deadline: Int,
    ...
    for statistics in collection.statistics() {
      ...
      // robust.
      let intervalMidpoint = statistics.startDate.addingTimeInterval(
      ...
      results.append(
        NewDataPoint(
          requestid: id, daystamp: daystamp, value: NSNumber(value: datapointValue),
