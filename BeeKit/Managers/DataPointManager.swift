...
public actor DataPointManager {
  ...
  private func updateDatapoint(goal: Goal, datapoint: DataPoint, datapointValue: NSNumber, comment: String) async throws
  {
    ...
    let _ = try await requestManager.put(
      url: "api/v1/users/{username}/goals/\(goal.slug)/datapoints/\(datapoint.id).json", parameters: params)
  }
  ...
  private func fetchDatapoints(goal: Goal, sort: String, per: Int, page: Int) async throws -> [DataPoint] {
    ...
    let response = try await requestManager.get(
      url: "api/v1/users/{username}/goals/\(goal.slug)/datapoints.json", parameters: params)
  ...
  public func updateToMatchDataPoints(goalID: NSManagedObjectID, healthKitDataPoints: [BeeDataPoint]) async throws {
    ...
    let datapoints = try await datapointsSince(
      goal: goal, daystamp: try! Daystamp(fromString: firstDaystamp.description))
      ...
      for (daystamp, dayDataPoints) in healthKitDataPointsByDay {
        group.addTask {
          let existingDatapointsForDay = await self.datapointsMatchingDaystamp(
            datapoints: realDatapoints, daystamp: daystamp)
          try await self.updateToMatchDataPointsForDay(
            goal: goal, newDataPoints: dayDataPoints, existingDatapoints: existingDatapointsForDay)
        }
      }
    }
  }
      ...
      for newDataPoint in newDataPoints {
        ...
        if let existingDatapoint = matchingDatapoint {
            ...
            || existingDatapoint.comment != newDataPoint.comment
          {
            group.addTask {
              self.logger.notice(
                "Updating datapoint for \(goal.id) with requestId \(newDataPoint.requestid, privacy: .public) from \(existingDatapoint.value) to \(newDataPoint.value)"
              )
              try await self.updateDatapoint(
                goal: goal, datapoint: existingDatapoint, datapointValue: newDataPoint.value,
          ...
          group.addTask {
            ...
            self.logger.notice(
              "Creating new datapoint for \(goal.id, privacy: .public) with requestId \(newDataPoint.requestid, privacy: .public): \(newDataPoint.value, privacy: .private)"
            )
