...
public class QuantityHealthKitMetric: HealthKitMetric {
  ...
  private func datapointsForCollection(
    ...
    for statistics in collection.statistics() {
      ...
      if daystamp < startDate || daystamp > endDate { continue }
      ...
      let value: Double? = {
        ...
        case .cumulative: return statistics.sumQuantity()?.doubleValue(for: unit)
        case .discrete: return statistics.minimumQuantity()?.doubleValue(for: unit)
        default: return nil
        }
      }()
