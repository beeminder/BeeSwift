...
class GoalManagerTests: XCTestCase {
  ...
  override func setUpWithError() throws {
    ...
    goalManager = GoalManager(
      requestManager: mockRequestManager, currentUserManager: currentUserManager, container: container)
  ...
  func testInitialGoalCreation() async throws {
    ...
    mockRequestManager.responses = [
      "api/v1/users/{username}.json": try JSONSerialization.jsonObject(
        with: userResponse.data(using: .utf8)!, options: []),
      "api/v1/users/{username}/goals.json": try JSONSerialization.jsonObject(
        with: goalsResponse.data(using: .utf8)!, options: []),
    ]
  ...
  func testGoalDeletion() async throws {
    ...
    mockRequestManager.responses = [
      "api/v1/users/{username}.json": try JSONSerialization.jsonObject(
        with: deletionResponse.data(using: .utf8)!, options: [])
    ]
  ...
  func testIncrementalUpdateUpdatesAllGoalsLastUpdatedLocal() async throws {
    ...
    mockRequestManager.responses = [
      "api/v1/users/{username}.json": try JSONSerialization.jsonObject(
        with: userResponse.data(using: .utf8)!, options: []),
      "api/v1/users/{username}/goals.json": try JSONSerialization.jsonObject(
        with: initialGoalsResponse.data(using: .utf8)!, options: []),
    ]
    ...
    mockRequestManager.responses = [
      "api/v1/users/{username}.json": try JSONSerialization.jsonObject(
        with: incrementalResponse.data(using: .utf8)!, options: [])
    ]
    ...
    XCTAssertGreaterThan(
      updatedGoalOne!.lastUpdatedLocal, originalGoalOneTimestamp, "Goal One should have updated timestamp")
    XCTAssertGreaterThan(
      updatedGoalTwo!.lastUpdatedLocal, originalGoalTwoTimestamp,
