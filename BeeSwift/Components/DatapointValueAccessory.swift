...
class DatapointValueAccessory: UIInputView {
  ...
  static func shouldShowAccessory() -> Bool {
    ...
    case .pad, .mac: return false
    case .phone, .carPlay, .tv, .vision, .unspecified: return true
    @unknown default: return true
    }
  }

  private func setupView() {
    if !showAccessory { return }
    ...
    registerForTraitChanges([UITraitUserInterfaceStyle.self]) {
      (self: Self, previousTraitCollection: UITraitCollection) in self.updateColorsToMatchMode()
    }
  }
