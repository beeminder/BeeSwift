...
class FreshnessIndicatorView: UIView {
  ...
  private struct TimeThreshold {
    static let recent: TimeInterval = 60 * 60  // Anything beyond will be considered "old"
  }
  ...
  private let styles: [Style] = [
    ...
    Style(backgroundColor: .Beeminder.gray, textColor: .label),
    // and the old ones
    Style(backgroundColor: .Beeminder.red, textColor: .label),
  ]
  ...
  private func fuzzyTimeString(for elapsed: TimeInterval) -> String {
    ...
    switch elapsed {
    case 0..<minute: return "Less than a minute ago"
    case minute..<(2 * minute): return "Less than 2 minutes ago"
    case (2 * minute)..<(5 * minute): return "Less than 5 minutes ago"
    case (5 * minute)..<(10 * minute): return "Less than 10 minutes ago"
    case (10 * minute)..<(15 * minute): return "Less than 15 minutes ago"
    case (15 * minute)..<(30 * minute): return "Less than 30 minutes ago"
    case (30 * minute)..<hour: return "Less than an hour ago"
    case hour..<(2 * hour): return "Less than 2 hours ago"
    case (2 * hour)..<day:
