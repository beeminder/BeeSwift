...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(
    ...
    UIBarButtonItem.appearance().setTitleTextAttributes(
      [NSAttributedString.Key.font: UIFont.beeminder.defaultFontPlain.withSize(18)], for: UIControl.State())
    ...
    NotificationCenter.default.addObserver(
      self, selector: #selector(self.handleGoalsUpdated), name: GoalManager.NotificationName.goalsUpdated, object: nil)
    NotificationCenter.default.addObserver(
      self, selector: #selector(self.handleUserSignedOut), name: CurrentUserManager.NotificationName.signedOut,
      object: nil)
    ...
    Task { @MainActor in
      ...
      do {
        let _ = try await ServiceLocator.signedRequestManager.signedPOST(
          url: "/api/private/device_tokens", parameters: parameters)
      } catch {
