  ...
  func perform() async throws -> some IntentResult & ProvidesDialog {
    ...
    do {
      let _ = try await ServiceLocator.requestManager.addDatapoint(
        urtext: "^ \(dataValue) \"\(dataComment)\"", slug: goalSlug)
...
extension IntentDialog {
  fileprivate static var valueParameterPrompt: Self {
    "What's the value of the datapoint?"
  }
  fileprivate static func goalParameterConfiguration(goal: String) -> Self {
    "\(goal)"
  }
  fileprivate static var goalParameterPrompt: Self {
    "Which goal?"
  }
