...
class EditGoalNotificationsViewController: EditNotificationsViewController {
  ...
  override func viewDidLoad() {
    ...
    self.useDefaultsSwitch.addTarget(
      self, action: #selector(EditGoalNotificationsViewController.useDefaultsSwitchValueChanged), for: .valueChanged)
    ...
    Task { @MainActor in
      ...
      do {
        let _ = try await self.requestManager.put(
          url: "api/v1/users/{username}/goals/\(self.goal.slug).json", parameters: params as [String: Any])
      ...
      if self.timePickerEditingMode == .deadline {
        ...
        do {
          ...
          let alert = UIAlertController(
            title: "Error saving to Beeminder", message: errorString, preferredStyle: .alert)
