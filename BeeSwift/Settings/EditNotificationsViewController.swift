...
class EditNotificationsViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    self.leadTimeStepper.addTarget(
      self, action: #selector(EditNotificationsViewController.leadTimeStepperValueChanged), for: .valueChanged)
    ...
    let tapGR = UITapGestureRecognizer(
      target: self, action: #selector(EditNotificationsViewController.alertstartLabelTapped))
    ...
    let deadlineTapGR = UITapGestureRecognizer(
      target: self, action: #selector(EditNotificationsViewController.deadlineLabelTapped))
  ...
  @objc func leadTimeStepperValueChanged() {
    ...
    self.leadTimeDelayTimer = Timer.scheduledTimer(
      timeInterval: 2, target: self, selector: #selector(self.sendLeadTimeToServer(_:)),
      userInfo: ["leadtime": NSNumber(value: self.leadTimeStepper.value as Double)], repeats: false)
  }
