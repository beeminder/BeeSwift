...
class ConfigureHKMetricViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    previewDescriptionLabel.attributedText = {
      ...
      text.append(
        NSMutableAttributedString(
          string: self.goal.slug, attributes: [NSAttributedString.Key.font: UIFont.beeminder.defaultBoldFont]))
      text.append(
        NSMutableAttributedString(
          string: " goal:", attributes: [NSAttributedString.Key.font: UIFont.beeminder.defaultFont]))
      return text
    }()
    ...
    Task { @MainActor in
      ...
      datapoints = try await self.metric.recentDataPoints(
        days: 5, deadline: self.goal.deadline, healthStore: self.healthStoreManager.healthStore,
      ...
      unitsLabel.attributedText = {
        ...
        text.append(
          NSMutableAttributedString(
            string: units.description, attributes: [NSAttributedString.Key.font: UIFont.beeminder.defaultBoldFont]))
        return text
      }()
    }
  }
    ...
    Task { @MainActor in
      ...
      do {
        let _ = try await self.requestManager.put(
          url: "api/v1/users/{username}/goals/\(self.goal.slug).json", parameters: params)
        hud.mode = .customView
        ...
        let alert = UIAlertController(
          title: "Error saving metric to Beeminder", message: errorString, preferredStyle: .alert)
