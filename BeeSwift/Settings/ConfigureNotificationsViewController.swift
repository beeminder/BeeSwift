...
class ConfigureNotificationsViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    NotificationCenter.default.addObserver(
      self, selector: #selector(self.foregroundEntered), name: UIApplication.willEnterForegroundNotification,
      object: nil)
  }
...
>
{
  ...
  func makeSnapshot() -> NSDiffableDataSourceSnapshot<Section, String> {
    ...
    snapshot.appendItems(
      goalsUsingNonDefaultNotifications.map { $0.slug },
      toSection: .goalsUsingCustomSettings)

    snapshot.appendSections([.goalsUsingDefaults])
    snapshot.appendItems(
      goalsUsingDefaultNotifications.map { $0.slug },
