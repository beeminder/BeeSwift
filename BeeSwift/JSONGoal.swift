...
class JSONGoal {
    ...
    func updateBeeminderWithValue(datapointValue : Double, daystamp : String, success: (() -> ())?, errorCompletion: (() -> ())?) {
        ...
        RequestManager.get(url: "api/v1/users/\(username)/goals/\(self.slug)/datapoints.json", parameters: params, success: { (response) in
                ...
                datapoints.forEach({ (datapoint) in
                    ...
                    if first {
                        ...
                        if datapointValue == val { success?() }
                            ...
                            RequestManager.put(url: "api/v1/users/\(username)/goals/\(self.slug)/datapoints/\(datapointID!).json", parameters: params, success: { (responseObject) in
                                success?()
                        ...
                        let datapointID = d["id"].string
                        RequestManager.delete(url: "api/v1/users/\(username)/goals/\(self.slug)/datapoints/\(datapointID!)", parameters: nil, success: { (response) in
                            //
                        }) { (error) in
                            //
                        }
