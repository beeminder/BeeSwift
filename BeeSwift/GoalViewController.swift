  ...
  UITextFieldDelegate, SFSafariViewControllerDelegate
{
  ...
  override func viewDidLoad() {
    ...
    self.valueTextField.addTarget(
      self, action: #selector(GoalViewController.valueTextFieldValueChanged), for: .editingChanged)
    ...
    self.valueStepper.addTarget(
      self, action: #selector(GoalViewController.valueStepperValueChanged), for: .valueChanged)
    ...
    if !self.goal.hideDataEntry {
      self.navigationItem.rightBarButtonItems?.append(
        UIBarButtonItem(
          image: UIImage(systemName: "stopwatch"), style: .plain, target: self,
          action: #selector(self.timerButtonPressed)))
    }

    NotificationCenter.default.addObserver(
      self, selector: #selector(onGoalsUpdatedNotification), name: GoalManager.NotificationName.goalsUpdated,
      object: nil)
  ...
  func datapointTableViewController(
    _ datapointTableViewController: DatapointTableViewController, didSelectDatapoint datapoint: BeeDataPoint
  ) {
  ...
  @objc func dateStepperValueChanged() {
    ...
    self.dateTextField.text = DateFormatter.dateTextFieldString(
      from: self.date, isDifferentYear: isDifferentYear, isDifferentMonth: isDifferentMonth)
  }
    ...
    Task { @MainActor in
      ...
      do {
        ...
        let alertController = UIAlertController(
          title: "Error", message: "Failed to add datapoint", preferredStyle: .alert)
  ...
  private static func makeInitialDateStepperValue(date: Date = Date(), for goal: Goal) -> Double {
    let daystampAccountingForTheGoalsDeadline = Daystamp(
      fromDate: date,
      deadline: goal.deadline)
    let daystampAssumingMidnightDeadline = Daystamp(
      fromDate: date,
      deadline: 0)
...
extension DateFormatter {
  ...
  fileprivate static func urtextDateString(from date: Date) -> String {
...
extension GoalViewController {
  ...
  private func createGoalMenu() -> UIMenu {
    ...
    let actions = options.map { option in
      UIAction(
        title: option.title, image: UIImage(systemName: option.imageSystemName),
        handler: { [weak self] _ in
          guard let self else { return }
          guard
            let link = option.action.makeLink(
              username: self.goal.owner.username, goalName: self.goal.slug, currentUserManager: self.currentUserManager)
          else { return }
