...
class EditDatapointViewController: UIViewController, UITextFieldDelegate {
  ...
  override func viewDidLoad() {
    ...
    formView.snp.makeConstraints { (make) in make.top.left.right.equalTo(self.view.safeAreaLayoutGuide).inset(10) }
    ...
    dateLabel.snp.makeConstraints { (make) in make.top.left.right.equalTo(formView).inset(10) }
  ...
  override func viewDidLayoutSubviews() {
    ...
    if let bgView = self.datePicker.subviews.first?.subviews.first?.subviews.first { bgView.backgroundColor = nil }
  }
  ...
  @objc func updateButtonPressed() {
    Task { @MainActor in
      ...
      do {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { self.navigationController?.dismiss(animated: true) }
      } catch {
  ...
  @objc func deleteButtonPressed() {
    ...
    alert.addAction(UIAlertAction(title: "Delete", style: .destructive, handler: { _ in self.deleteDatapoint() }))
    alert.addAction(
      UIAlertAction(
        title: "Cancel",
        style: .cancel,
